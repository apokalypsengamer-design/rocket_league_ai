...
class Brain:
    ...
    def _print_line(self, state: GameState, action: Action) -> None:
        ...
        player_str = f"P({state.player_x:.2f},{state.player_y:.2f})"
        ...
        log.debug(
            ...
            f"player=({state.player_x:.2f},{state.player_y:.2f}) "
            f"shot={state.shot_opportunity} action={keys} "
            f"boost={boost_str.strip()} "
            f"ball=({state.ball_x:.2f},{state.ball_y:.2f})"
        )
