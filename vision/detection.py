...
class Detector:
    def __init__(self, config: Config):
        ...
        self._goal_own_lower    = np.array(self._cfg.goal_own_hsv_lower,   dtype=np.uint8)
        self._goal_own_upper    = np.array(self._cfg.goal_own_hsv_upper,   dtype=np.uint8)
        self._goal_enemy_lower  = np.array(self._cfg.goal_enemy_hsv_lower, dtype=np.uint8)
        self._goal_enemy_upper  = np.array(self._cfg.goal_enemy_hsv_upper, dtype=np.uint8)
        self._boost_pad_lower   = np.array(self._cfg.boost_pad_hsv_lower,  dtype=np.uint8)
        self._boost_pad_upper   = np.array(self._cfg.boost_pad_hsv_upper,  dtype=np.uint8)
        self._enemy_lower       = np.array(self._cfg.enemy_hsv_lower,      dtype=np.uint8)
        self._enemy_upper       = np.array(self._cfg.enemy_hsv_upper,      dtype=np.uint8)
        self._teammate_lower    = np.array(self._cfg.teammate_hsv_lower,   dtype=np.uint8)
        self._teammate_upper    = np.array(self._cfg.teammate_hsv_upper,   dtype=np.uint8)
    ...
    def detect(self, processed: np.ndarray, raw: np.ndarray) -> GameState:
        ...
        enemies              = self._find_agents(hsv, self._enemy_lower,    self._enemy_upper)
        teammates            = self._find_agents(hsv, self._teammate_lower, self._teammate_upper)
    ...
    def _dummy_state(self, raw: np.ndarray, processed: np.ndarray) -> GameState:
        ball = self._find_ball(processed, to_hsv(processed))
        boost = self._read_boost(processed)
        phase = self._determine_phase(ball, boost)

        dummy_enemies   = [ObjectPosition(0.7, 0.3, True), ObjectPosition(0.3, 0.2, True)]
        dummy_teammates = [ObjectPosition(0.5, 0.8, True)]
        dummy_pads = [
            ObjectPosition(0.1, 0.5, True), ObjectPosition(0.9, 0.5, True),
            ObjectPosition(0.5, 0.1, True), ObjectPosition(0.5, 0.9, True),
        ]
    ...
    def _find_goals(self, frame: np.ndarray, hsv: np.ndarray) -> tuple[ObjectPosition, ObjectPosition]:
        own   = self._largest_blob(hsv, self._goal_own_lower,   self._goal_own_upper,   frame, self._cfg.goal_min_area)
        enemy = self._largest_blob(hsv, self._goal_enemy_lower, self._goal_enemy_upper, frame, self._cfg.goal_min_area)
        if not own.visible:
    ...
    def _find_boost_pads(self, frame: np.ndarray, hsv: np.ndarray) -> list[ObjectPosition]:
        return self._all_blobs(hsv, self._boost_pad_lower, self._boost_pad_upper,
                               frame, self._cfg.boost_pad_min_area, self._cfg.max_boost_pads)

    def _find_agents(self, hsv: np.ndarray, lower: np.ndarray, upper: np.ndarray) -> list[ObjectPosition]:
        h, w = hsv.shape[:2]
        ...
        for c in sorted(contours, key=cv2.contourArea, reverse=True)[:self._cfg.max_agents]:
            ...
            if M["m00"] == 0:
                continue
            result.append(ObjectPosition(M["m10"] / M["m00"] / w, M["m01"] / M["m00"] / h, True))
        return result

    def _largest_blob(self, hsv: np.ndarray, lower: np.ndarray, upper: np.ndarray,
    ...
    def _all_blobs(self, hsv: np.ndarray, lower: np.ndarray, upper: np.ndarray,
    ...
    @staticmethod
    def _nearest_pad(pads: list[ObjectPosition], px: float, py: float) -> ObjectPosition | None:
